<?php
/**
 * Mgsisk\Webcomic\Collection\Widget\WebcomicCollectionLink class
 *
 * @package Webcomic
 */
...
class WebcomicCollectionLink extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			__( 'Webcomic Collection Link', 'webcomic' ),
			[
				'description' => __( 'Display a link to a comic collection.', 'webcomic' ),
			]
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $instance['relation'] ) {
			$link_args['relation'] = $instance['relation'];
		}

		$output = get_webcomic_collection_link( $instance['link'], $instance['collection'], null, $link_args );
	...
	public function update( $new, $old ) {
		if ( ! in_array( $new['relation'], [ '', 'first', 'last', 'random' ], true ) ) {
			$new['relation'] = '';
		}
	...
	public function form( $instance ) {
		...
		require __DIR__ . '/class-webcomiccollectionlink-inc-form.php';
	...
	public function hook_add_media_state( array $states ) : array {
		...
		$states[] = __( 'Webcomic Collection Link Widget Image', 'webcomic' );
