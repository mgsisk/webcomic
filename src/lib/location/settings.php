...
function settings() {
	...
	add_filter( 'webcomic_activate_location', __NAMESPACE__ . '\hook_activate' );
	add_filter( 'webcomic_deactivate_location', __NAMESPACE__ . '\hook_deactivate' );
	...
	foreach ( webcomic( 'option.collections' ) as $collection ) {
		...
		add_filter( "{$collection}_location_pre_add_form", "{$namespace}\hook_add_sorter_link" );
	}
}
...
function hook_add_allowed_options( array $allowed ) : array {
	return add_allowed_options( 'location', $allowed );
}
...
function hook_new_collection_taxonomy( array $defaults ) : array {
	return new_collection_taxonomy( 'location', $defaults );
}
...
function hook_remove_sorter_submenu_page() {
	remove_sorter_submenu_page( 'location' );
}
...
function hook_add_overview_help() {
	add_overview_help( 'location', __NAMESPACE__ );
}
...
function hook_add_settings_help() {
	add_settings_help( 'location', __NAMESPACE__ );
}
...
function hook_sanitize_field_slug( array $options ) : array {
	return sanitize_field_slug( 'location', $options );
}
...
function hook_sanitize_field_behavior( array $options ) : array {
	return sanitize_field_behavior( 'location', $options );
}
