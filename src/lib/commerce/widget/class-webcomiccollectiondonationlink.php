<?php
/**
 * Mgsisk\Webcomic\Commerce\Widget\WebcomicCollectionDonationLink class
 *
 * @package Webcomic
 */
...
class WebcomicCollectionDonationLink extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			__( 'Webcomic Collection Donation Link', 'webcomic' ),
			[
				'description' => __( 'Display a link to a comic collection donation.', 'webcomic' ),
			]
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$output = get_webcomic_collection_donation_link( $instance['link'], $instance['collection'] );
	...
	public function form( $instance ) {
		...
		require __DIR__ . '/class-webcomiccollectiondonationlink-inc-form.php';
	...
	public function hook_add_media_state( array $states ) : array {
		...
		$states[] = __( 'Webcomic Collection Donation Link Widget Image', 'webcomic' );
	...
	protected function get_instance_defaults() : array {
		return [
			'title'      => '',
			'media'      => 0,
			'link'       => esc_html__( 'Support %title', 'webcomic' ),
			'collection' => '',
		];
	}
}
