...
function get_the_chapter($chapter=false){
	...
	$output['pages']       = ($chapter->parent) ? (int) $chapter->count : (int) $page_count;
...
function the_comic($size=false){
	...
	echo '<img src="'.get_comic_image($comic,$size).'" alt="'.$comic['title'].'" title="'.$comic['description'].'" class="'.$comic['class'].'" />';
}
 ...
 */
function comics_nav_link($sep=' &bull; ',$fstlabel='',$prelabel='',$nxtlabel='',$lstlabel=''){
	$midsep = $sep;
	
	$fstlabel = ($fstlabel) ? $fstlabel : __('&laquo; First','webcomic');
	$prelabel = ($prelabel) ? $prelabel : __('&lt; Back','webcomic');
	$nxtlabel = ($nxtlabel) ? $nxtlabel : __('Next &gt;','webcomic');
	$lstlabel = ($lstlabel) ? $lstlabel : __('Last &raquo;','webcomic');
 ...
 */
function first_comic_link($label=''){
 ...
 * @param   str $label The text to display for the link.
 */
function last_comic_link($label=''){
 ...
 * @param str $label The text to display for the link.
 */
function previous_comic_link($label=''){
	global $wp_query;
 ...
 * @param str $label The text to display for the link.
 */
function next_comic_link($label=''){
	global $wp_query;
...
function the_chapter($label=''){
	...
	if($chapter)
		echo '<a href="'.$chapter['link'].'" title="'.sprintf(__('Go to the beginning of %s','webcomic'),$chapter['title']).'">'.$label.'</a>';
}
 ...
 * @uses    get_the_chapter()
 */
function the_volume($label=''){
	load_webcomic_domain();
	
	$volume = get_the_chapter('volume');
	$label = ($label) ? $label : $volume['title'];
	
	if($volume)
		echo '<a href="'.$volume['link'].'" title="'.sprintf(__('Go to the beginning of %s','webcomic'),$volume['title']).'">'.$label.'</a>';
}

/**
 * Displays the 'current' chapter link.
 * 
 * This is a shortcut function for displaying a link to the beginning
 * of the 'current' chapter as defined by the Current Chapter setting.
 * 
 * @package WebComic
 * @since   1.7
 * 
 * @uses get_the_chapter()
 */
function the_current_chapter($label=''){
	load_webcomic_domain();
	
	$chapter = get_the_chapter(get_comic_current_chapter());
	$label = ($label) ? $label : __('Current Chapter','webcomic');
...
/**
 * Displays the 'current' volume link.
 * 
 * This is a shortcut function for displaying a link to the beginning
 * of the 'current' volume as defined by the Current Chapter setting.
 * 
 * @package WebComic
 * @since   1.7
 * 
 * @uses get_the_chapter()
 */
function the_current_volume($label=''){
	load_webcomic_domain();
	
	$chapter = get_the_chapter(get_comic_current_chapter());
	$volume = get_the_chapter($chapter['parent']);
	$label = ($label) ? $label : __('Current Volume','webcomic');
