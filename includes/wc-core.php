...
function get_the_comic( $id = false, $limit = false, $chapter = false ) {
	...
	if ( is_file( get_comic_directory( 'abs', false, $comic_dir ) . get_post_meta( $comic_post->ID, 'comic_file', true ) ) ) {
		...
		$output->file      = get_comic_directory( 'url', false, $comic_dir ) . get_post_meta( $comic_post->ID, 'comic_file', true );
		$output->file_data = ( object ) getimagesize( $output->file );
		$output->flash     = ( 'application/x-shockwave-flash' == $output->file_data->mime ) ? true : false;
		
		if ( get_post_meta( $comic_post->ID, 'comic_large', true ) ) {
			$output->large      = get_comic_directory( 'url', true, $comic_dir ) . get_post_meta( $comic_post->ID, 'comic_large', true );
			$output->large_data = ( object ) getimagesize( $output->large );
		}
		
		if ( get_post_meta( $comic_post->ID, 'comic_medium', true ) ) {
			$output->medium      = get_comic_directory( 'url', true, $comic_dir ) . get_post_meta( $comic_post->ID, 'comic_medium', true );
			$output->medium_data = ( object ) getimagesize( $output->medium );
		}
		
		if ( get_post_meta( $comic_post->ID, 'comic_thumb', true ) ) {
			$output->thumb      = get_comic_directory( 'url', true, $comic_dir ) . get_post_meta( $comic_post->ID, 'comic_thumb', true );
			$output->thumb_data = ( object ) getimagesize( $output->thumb );
		}	
	} else {
		...
		foreach( array_keys( $comic_files ) as $key ) {
			if ( false !== strpos( basename( $comic_files[ $key ] ), $comic_name ) ){
				$output->file = get_comic_directory( 'url', 0, $comic_dir ) . basename( $comic_files[ $key ] );
				break;
			}
		}
		...
		$output->file_data = ( object ) getimagesize( $file );
		$output->flash     = ( 'application/x-shockwave-flash' == $output->file_data->mime ) ? true : false;
		...
		foreach ( $comic_thumbs as $comic_thumb ) {
			...
			if ( strpos( $comic_thumb, 'thumb' ) && !$output->thumb ) {
				...
				$output->humb_data = ( object ) getimagesize( $output->thumb );
			}
		}
	}
 ...
 */
function get_comic_object(  $comic = false, $size = false ) {
...
function transcript_template( $file = '/transcript.php' ) {
	...
	if ( file_exists( TEMPLATEPATH . $file ) )
		...
		require( ABSPATH . PLUGINDIR . '/webcomic/includes/transcript.php' );
}
