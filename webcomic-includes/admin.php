...
class webcomic_admin extends webcomic {
	...
	public function hook_edited_webcomic_collection( $term_id, $tt_id ) {
		...
		if ( !is_readable( $oabs ) ) {
			if ( !@mkdir( "$nabs/thumbs", 0755, true ) )
				$this->errors[ 'no_directory' ] = sprintf( __( 'The directory for this collection could not be found. You will need to create %s/thumbs', 'webcomic' ), $oabs, $nabs );
		} else if ( $nabs != $oabs and !@rename( $oabs, $nabs ) )
	...
	public function admin_metabox( $post ) {
		...
		<input type="hidden" name="webcomic_ajax" value="0">
		<?php require_once $this->dir . '/webcomic-includes/metabox.js';
	}
