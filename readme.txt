...
If building themes isn't your thing, check out [the official plugin website](http://maikeruon.com/wcib/) for a collection of WordPress themes that are specifically designed to fully utilize WebComic, including the highly customizable InkBlot, the skeletal starter Archimedies, and the technically advanced Hydrogen.
...
- Core updates:
	...
	- **New** Template tags: get\_comic\_buffer, the\_comic\_buffer, the\_comic\_series. the\_comic\_embed has a new format option which allows you to specify HTML or BBCode output.
	...
	- **New** Widget Updates. All widgets have been updated to use the WP\_Widgets class
	...
	- get\_the\_comic() no longer returns false when a comic file can't be found, but the 'file' property is set to false.
	- Internal 1.x to 2.0 upgrade functions have been removed. Users may still be able to upgrade directly from Webcomic 1.x to 2.1, though certain settings may be reset to their defaults in the process.
