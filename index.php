...
?>
	...
	<?php if(have_posts()): while (have_posts()): the_post(); if(in_comic_category()): //And then show the comic post ?>
		<div <?php post_class('comic-series-'.get_post_comic_category()); ?> id="post-<?php the_id(); ?>">
			...
			<p class="date"><?php the_time(get_option('date_format')); ?></p>
			<div class="content"><?php the_content(); ?></div>
			<?php the_comic_transcript(); if(get_option('comic_embed_code')): ?><div class="embed-title"><span><?php _e('Share This Comic','inkblot') ?></span></div><?php the_comic_embed(get_option('comic_embed_code_size')); endif; ?>
			<p class="meta">
...
?>
	...
	<?php /* If this is a tag archive */}elseif(is_tag()){ ?><h1><q><?php single_tag_title(); ?></q><?php _e(' Posts','inkblot'); ?></h1>
	<?php /* If this is a chapter archive */}elseif(is_tax()){ ?><h1><?php single_chapter_title(); ?><?php _e(' Comics','inkblot'); ?></h1>
...
?>
	...
	<p class="clear"><?php _e('Or try your luck with a ','inkblot'); random_comic('ranomd comic'); ?>.</p>
<?php
	endif; //end 404.php
	get_footer();
?>